...
output_dir = datasets_path + dataset + "mask_yolo/"
...
for i in tqdm(range(len(image_paths) - 1)):
    ...
    # Using yolo11n-seg
    for idx, mask in enumerate(masks_yolo):
        ...
        if idx != 0:
            class_id = int(result_yolo.boxes.cls[idx])
            class_name = result_yolo.names[class_id]
            is_dynamic = (class_name in dynamic_classes) or (flow_diff > flow_diff_thres)
        else:
            is_dynamic = (flow_diff > yolo_flow_thres)
