     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    if(mask.channels() == 4){
        cv::cvtColor(mask, maskGray, cv::COLOR_BGRA2GRAY);
    }
    ...
    if (M<9000 && M!=0){
        ...
        for(size_t i=0; i<M; i++){
            ...
            int y = static_cast<int>(std::floor(mvKeys[i].pt.y));

            if(x<0 || x+1 >= maskGray.cols || y<0 || y+1>=maskGray.rows){
                continue;
            }

            bool isDynamic = (maskGray.at<uchar>(y,x)==255 ||
                            maskGray.at<uchar>(y+1,x)==255 ||
                            maskGray.at<uchar>(y,x+1)==255 ||
                            maskGray.at<uchar>(y+1,x+1)==255);
            if(!isDynamic){
                _mvKeys.push_back(mvKeys[i]);
                _mDescriptors.push_back(mDescriptors.row(static_cast<int>(i)));
            }
        }
        std::cout<< "Erase featrues number ="<<  mvKeys.size()-_mvKeys.size() << std::endl;
